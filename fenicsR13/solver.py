...
class Solver:
    ...
    def solve(self):
        ...
        print("Modified ...")
        #theta_out = df.XDMFFile(self.mesh.mpi_comm(),"{0}theta.xdmf".format(self.output_folder))
        #theta_out.write_checkpoint(self.sol["theta"], "theta", 0, df.XDMFFile.Encoding.HDF5, False)
        #s_out = df.XDMFFile(self.mesh.mpi_comm(),"{0}s.xdmf".format(self.output_folder))
        #s_out.write_checkpoint(self.sol["s"], "s", 0, df.XDMFFile.Encoding.HDF5, False)
        #u_out = df.XDMFFile(self.mesh.mpi_comm(),"{0}u.xmdf".format(self.output_folder))
        #u_out.write_checkpoint(self.sol["u"], "u", 0, df.XDMFFile.Encoding.HDF5, False)
        #theta_out.close()
        #s_out.close()
        #u_out.close()
        p_out = df.XDMFFile(self.mesh.mpi_comm(),"{0}p.xdmf".format(self.output_folder))
        p_out.write_checkpoint(self.sol["p"], "p", 0, df.XDMFFile.Encoding.HDF5, False)
        sigma_out = df.XDMFFile(self.mesh.mpi_comm(),"{0}sigma.xdmf".format(self.output_folder))
        sigma_out.write_checkpoint(self.sol["sigma"], "sigma", 0, df.XDMFFile.Encoding.HDF5, False)
        p_out.close()
        sigma_out.close()
        self.postprocess_crooks3d()
    ...
    def postprocess_crooks3d(self):
        ...
        pressure_tensor = df.as_matrix([
            [self.sol["sigma"][0,0]+self.sol["p"], self.sol["sigma"][0,1],self.sol["sigma"][0,2]],
            [self.sol["sigma"][1,0], self.sol["sigma"][1,1]+self.sol["p"],self.sol["sigma"][1,2]],
            [self.sol["sigma"][2,0], self.sol["sigma"][2,1],self.sol["sigma"][2,2]+self.sol["p"]],
            ])
        ...
        #
        print("Right vane")
        print("RH : f_x, f_y, f_z: {}, {}, {}, Right Hot".format(ff(rh_f[0]), ff(rh_f[1]), ff(rh_f[2])))
        print("RC : f_x, f_y, f_z: {}, {}, {}, Right Cold" .format(ff(rc_f[0]), ff(rc_f[1]), ff(rc_f[2])))
        print("NrF: f_x, f_y, f_z: {}, {}, {}, Normal Force".format(ff(rh_f[0]+rc_f[0]), ff(rh_f[1]+rc_f[1]), ff(rh_f[2]+rc_f[2])))
        print("RT : f_x, f_y, f_z: {}, {}, {}, Right Transition".format(ff(rt_f[0]), ff(rt_f[1]), ff(rt_f[2])))
        print("NF : f_x, f_y, f_z: {}, {}, {}, Net Force".format(ff(rh_f[0]+rc_f[0]+rt_f[0]), ff(rh_f[1]+rc_f[1]+rt_f[1]), ff(rh_f[2]+rc_f[2]+rt_f[2])))

        with open("{0}force_calculation.txt".format(self.output_folder), "w") as forcefile:
            ...
            forcefile.write("NFr: f_x, f_y, f_z: {}, {}, {}, Normal Force\n".format(ff(lh_f[0]+lc_f[0]), ff(lh_f[1]+lc_f[1]), ff(lh_f[2]+lc_f[2])))
            forcefile.write("LT : f_x, f_y, f_z: {}, {}, {}, Left Transition\n".format(ff(lt_f[0]), ff(lt_f[1]), ff(lt_f[2])))
            forcefile.write("NF : f_x, f_y, f_z: {}, {}, {}, Net Force\n".format(ff(lh_f[0]+lc_f[0]+lt_f[0]), ff(lh_f[1]+lc_f[1]+lt_f[1]), ff(lh_f[2]+lc_f[2]+lt_f[2])))
            ...
            forcefile.write("NFr: f_x, f_y, f_z: {}, {}, {}, Normal Force\n".format(ff(uh_f[0]+uc_f[0]), ff(uh_f[1]+uc_f[1]), ff(uh_f[2]+uc_f[2])))
            forcefile.write("UT : f_x, f_y, f_z: {}, {}, {}, Up Transition\n".format(ff(ut_f[0]), ff(ut_f[1]), ff(ut_f[2])))
            forcefile.write("NF : f_x, f_y, f_z: {}, {}, {}, Net Force\n".format(ff(uh_f[0]+uc_f[0]+ut_f[0]), ff(uh_f[1]+uc_f[1]+ut_f[1]), ff(uh_f[2]+uc_f[2]+ut_f[2])))
            ...
            forcefile.write("NrF: f_x, f_y, f_z: {}, {}, {}, Normal Force\n".format(ff(dh_f[0]+dc_f[0]), ff(dh_f[1]+dc_f[1]), ff(dh_f[2]+dc_f[2])))
            forcefile.write("DT : f_x, f_y, f_z: {}, {}, {}, Down Transition\n".format(ff(dt_f[0]), ff(dt_f[1]), ff(dt_f[2])))
            forcefile.write("NF : f_x, f_y, f_z: {}, {}, {}, Net Force\n".format(ff(dh_f[0]+dc_f[0]+dt_f[0]), ff(dh_f[1]+dc_f[1]+dt_f[1]), ff(dh_f[2]+dc_f[2]+dt_f[2])))
