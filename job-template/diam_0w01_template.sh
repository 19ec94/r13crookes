...
### Job name
#SBATCH --job-name=diam-0w01-{JOB_PARAM}

### File / path where STDOUT will be written, %J is the job id
#SBATCH --output=diam-0w01-{JOB_PARAM}-job-out.%J
...
JOB_NAME="diam_0w01_{JOB_PARAM}"
cd $HOME/fenicsR13
#
#### Execute the container with commands directly
$MPIEXEC apptainer exec ./fenicsR13.sif /bin/bash -c "cd 3d_crooks && fenicsR13 input/diam/0w01/input_${JOB_NAME}.yml"
