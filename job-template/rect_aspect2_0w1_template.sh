...
### Job name
#SBATCH --job-name=rect-aspect2-0w1-{JOB_PARAM}

### File / path where STDOUT will be written, %J is the job id
#SBATCH --output=rect-aspect2-0w1-{JOB_PARAM}-job-out.%J
...
JOB_NAME="rect_aspect2_0w1_{JOB_PARAM}"
cd $HOME/fenicsR13
#
#### Execute the container with commands directly
$MPIEXEC apptainer exec ./fenicsR13.sif /bin/bash -c "cd 3d_crooks && fenicsR13 input/rect/0w1/input_${JOB_NAME}.yml"
